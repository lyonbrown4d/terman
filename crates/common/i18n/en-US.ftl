...
builtin-htop-tree-collapse = Collapse
builtin-htop-tree-expand = Expand
...
builtin-htop-footer-help = Help
builtin-htop-footer-search = Search
builtin-htop-footer-filter = Filter
builtin-htop-footer-priority-higher = Nice-
builtin-htop-footer-priority-lower = Nice+
builtin-htop-footer-kill = Kill
builtin-htop-footer-delay = Delay
builtin-htop-footer-quit = Quit
builtin-htop-footer-yes = Yes
builtin-htop-footer-no = No
builtin-htop-footer-search-prompt = type search, Enter jump, Esc cancel
builtin-htop-footer-filter-prompt = type filter, Enter apply, Esc cancel
builtin-htop-footer-tree-prompt = arrows select, +/- branch, * all
builtin-htop-footer-select-prompt = arrows select, +/- delay
builtin-htop-footer-view-flat = Flat
...
builtin-screen-attach-help = screen keys: Ctrl-A c/Ctrl-C new window | Ctrl-A d/Ctrl-D detach | Ctrl-A D detach all | Ctrl-A k/Ctrl-K kill current window (confirm) | Ctrl-A C clear | Ctrl-A b/Ctrl-B blank | Ctrl-A Z reset | Ctrl-A r wrap | Ctrl-A S/| split region | Ctrl-A Tab focus region | Ctrl-A X remove region | Ctrl-A Q only region | Ctrl-A l redisplay | Ctrl-A m/Ctrl-M last message | Ctrl-A M monitor, Ctrl-A _ silence | Ctrl-A h hardcopy | Ctrl-A H log | Ctrl-A . dumptermcap | Ctrl-A ]/Ctrl-] paste buffer | Ctrl-A </>/= exchange buffer | Ctrl-A q xon | Ctrl-A s xoff | Ctrl-A i/Ctrl-I info | Ctrl-A n/Ctrl-N/Space next | Ctrl-A N number | Ctrl-A p/Ctrl-P/Backspace previous | Ctrl-A 0..9 select window | Ctrl-A ' select by number/title | Ctrl-A * displays | Ctrl-A t/Ctrl-T time | Ctrl-A v version | Ctrl-A , license | Ctrl-A F fit | Ctrl-A W width | Ctrl-A w/Ctrl-W windows | Ctrl-A " windowlist (arrows/Enter/Esc) | Ctrl-A \\ quit (confirm) | Ctrl-A ? help | Ctrl-A A title | Ctrl-A : command | Ctrl-A Ctrl-A last window | Ctrl-A a send literal Ctrl-A | Mouse wheel next/previous | right-click windows | middle-click help | Ctrl-A [/Esc copy mode (/? find, n/N repeat) | Ctrl-A - blank region
builtin-screen-monitor-status =
    { $state ->
        [on] screen: monitoring window {$window}
       *[off] screen: no longer monitoring window {$window}
    }
builtin-screen-monitor-activity = screen: activity in window {$window} ({$title})builtin-screen-silence-status =
    { $state ->
        [on] screen: silence monitoring enabled for window {$window} ({$seconds}s)
       *[off] screen: silence monitoring disabled for window {$window}
    }
...
builtin-screen-control-monitor-required = Use screen -X monitor [on|off|toggle].builtin-screen-control-silence-required = Use screen -X silence [seconds|on|off|toggle].
...
builtin-tmux-attach-help = tmux keys: Ctrl-B c new window | Ctrl-B d detach | Ctrl-B %/" split pane | Ctrl-B o next pane | Ctrl-B ; last pane | Ctrl-B q choose pane | Ctrl-B arrows select pane | Ctrl-B Ctrl-arrows resize pane | Ctrl-B {/} swap pane up/down | Ctrl-B z zoom pane | Ctrl-B Space next layout | Ctrl-B x kill pane | Ctrl-B & kill window | Ctrl-B , rename window | Ctrl-B $ rename session | Ctrl-B n/p next/previous | Ctrl-B l last window | Ctrl-B f find window | Ctrl-B 0..9 select window | Ctrl-B ? help | Mouse: status click/wheel switch, right-click windows, middle-click help | Ctrl-B [ copy mode | Ctrl-B ] paste buffer | Ctrl-B : command prompt.
...
builtin-tmux-pane-size-required = specify pane size with resize-pane -x <cols> -y <rows> or move a boundary with -L/-R/-U/-D [adjustment]
...
builtin-tmux-copy-status = tmux copy mode | find {$search} | line {$line}/{$total} | arrows/PgUp/PgDn move | /? find, n/N repeat | Space/Enter mark | Esc cancel
builtin-tmux-copy-selection-status = tmux copy selection | find {$search} | line {$line}/{$total} | arrows/PgUp/PgDn move | /? find, n/N repeat | Enter copy | Esc cancel
...
builtin-screen-wrap-status = screen wrap for window {$window}: {$state}
builtin-screen-control-wrap-required = Use screen -X wrap [on|off|toggle].
