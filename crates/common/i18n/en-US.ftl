...
builtin-screen-control-command-unsupported = Unsupported screen control command `{$command}`. Currently supported: quit, kill, bell, help, echo, wall, stuff, screen, paste, pastefile, readbuf, writebuf, source, detach, pow_detach, resize, select, next, prev, scrollback, defscrollback, logfile, log, title, aka, sleep, time, version, info, displays, windows, hardcopy, clear, reset, eval, at, colon, sessionname.
