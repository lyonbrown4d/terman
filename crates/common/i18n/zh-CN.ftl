...
builtin-screen-attach-help = screen 快捷键：Ctrl-A d 断开连接 | Ctrl-A D 断开全部连接 | Ctrl-A k 结束会话 | Ctrl-A C 清屏 | Ctrl-A Z 重置终端 | Ctrl-A r 同步尺寸 | Ctrl-A h 生成 hardcopy | Ctrl-A i 显示信息 | Ctrl-A * 显示 displays | Ctrl-A w 显示窗口 | Ctrl-A ? 显示帮助 | Ctrl-A Ctrl-A 发送字面 Ctrl-A
