...
builtin-screen-attach-help = screen 快捷键：Ctrl-A c/Ctrl-C 新建窗口 | Ctrl-A d/Ctrl-D 断开连接 | Ctrl-A D 断开全部连接 | Ctrl-A k/Ctrl-K 结束当前窗口 | Ctrl-A C 清屏 | Ctrl-A Z 重置终端 | Ctrl-A r 同步尺寸 | Ctrl-A l 重绘 | Ctrl-A h 生成 hardcopy | Ctrl-A H 切换日志 | Ctrl-A . 生成 termcap | Ctrl-A ]/Ctrl-] 粘贴 paste buffer | Ctrl-A q 发送 xon | Ctrl-A s 发送 xoff | Ctrl-A i 显示信息 | Ctrl-A n/Ctrl-N/Space 下一个窗口 | Ctrl-A p/Ctrl-P/Backspace 上一个窗口 | Ctrl-A 0..9 选择窗口 | Ctrl-A * 显示 displays | Ctrl-A t 显示时间 | Ctrl-A v 显示版本 | Ctrl-A , 显示 license | Ctrl-A F 适配尺寸 | Ctrl-A W 切换宽度 | Ctrl-A w/Ctrl-W 显示窗口 | Ctrl-A " 显示 windowlist | Ctrl-A ? 显示帮助 | Ctrl-A Ctrl-A 上一个活动窗口 | Ctrl-A a 发送字面 Ctrl-A
