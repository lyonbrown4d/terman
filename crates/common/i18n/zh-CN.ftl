...
builtin-htop-help = F1 帮助，Tab/左/右 切换标签，方向键选择，PgUp/PgDn 滚动，1-4 跳转标签，F3 查找，F4 或 / 过滤，F5/t 树视图，F6/s 排序菜单，F 跟随选中 PID，I 反向排序，F9 选择信号，F10/q/Esc 退出，+/- 调整刷新间隔。鼠标：点击标签/页脚/Overview 或 Processes 进程行/表头，滚轮滚动列表、详情、I/O 或网络页，右键进程行打开信号菜单。F7/F8 调整优先级。
...
builtin-screen-control-monitor-required = 请使用 screen -X monitor [on|off|toggle]。builtin-screen-control-logfile-required = 请按 screen -X logfile <路径> 指定日志文件路径，或按 screen -X logfile flush <秒数> 指定刷新间隔。
...
builtin-tmux-attach-help = tmux 快捷键：Ctrl-B c 新建窗口 | Ctrl-B d 断开连接 | Ctrl-B %/" 拆分 pane | Ctrl-B o 切换 pane | Ctrl-B {/} 向上/向下交换 pane | Ctrl-B z 缩放 pane | Ctrl-B x 结束 pane | Ctrl-B & 结束窗口 | Ctrl-B , 重命名窗口 | Ctrl-B n/p 前后切换 | Ctrl-B l 上一个窗口 | Ctrl-B 0..9 选择窗口 | Ctrl-B ? 帮助 | 鼠标：状态栏点击/滚轮切换，右键显示窗口列表，中键显示帮助 | Ctrl-B [ 复制模式 | Ctrl-B ] 粘贴 buffer
