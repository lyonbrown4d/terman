...
pub(super) fn request_wrap_command(
    args: &ScreenArgs,
    inline_payload: &str,
    extra_args: &[String],
) -> io::Result<()> {
    ...
    let enabled = match payload.trim().to_ascii_lowercase().as_str() {
        ...
        _ => {
            return Err(io::Error::new(
                ...
                terman_common::builtin_screen_control_wrap_required_hint(),
            ));
        }
    };
    send_targeted_session_control_request(args, ScreenIpcRequest::SetWrap { enabled })
}
